...
_add_array:
	...
	movl $0,%eax  # sum = 0
	movq $0,%rdx  # i = 0 (use a 64-bit register, see hint above)
	movl $1,%ecx
	subl %ecx, %esi

/* THIS IS THE TOP OF THE LOOP, NEED A LABEL */
TOP:
	cmpl %esi,%edx # compare i to size ( backwards )
	jg DONE   #   if i is not less than size, jump out of loop

	addl (%rdi, %rdx, 4),%eax  # sum += a[i]
	incl %edx  # i++		
